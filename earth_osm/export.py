...
def convert_pd_to_gdf(pd_df):
    ...
    if not "Type" in pd_df.columns:
        raise KeyError(
            "pandas dataframe does not have a Type ('node', 'way' or 'area') column"
        )
    # unstringify lonlat column if necessary
    pd_df["lonlat"] = pd_df["lonlat"].apply(
...
class EarthOSMWriter:
    ...
    def __init__(self, region_list, primary_name, feature_list, data_dir, out_format):
        ...
        logger.debug(
            f"File writer initialized with region_list: {region_list}, primary_name: {primary_name}, feature_list: {feature_list}"
        )
