...
def setup_extract_parser(subparsers):
    ...
    extract_parser.add_argument(
        "primary", choices=get_primary_list(), type=str, help="Primary Feature"
    )
    extract_parser.add_argument(
        "--regions", nargs="+", type=str, required=True, help="Region Identifier(s)"
    )
    ...
    agg_group.add_argument(
        "--agg_feature", action="store_true", help="Aggregate Outputs by feature"
    )
    agg_group.add_argument(
        "--agg_region", action="store_true", help="Aggregate Outputs by region"
    )
...
def validate_regions(regions: List[str]):
    ...
    if invalid_regions:
        raise ValueError(
            f'Invalid Region(s): {", ".join(invalid_regions)}. Run "earth_osm view regions" to view valid regions.'
        )
...
def validate_features(primary: str, features: List[str]):
    if features:
        ...
        if invalid_features:
            raise ValueError(
                f'Invalid Feature(s): {", ".join(invalid_features)}. Run "earth_osm view features" to view valid features.'
            )
        return features
    return list(get_feature_list(primary))
